...
void parse_input(string test_input) {
  return;
}

namespace util {

void interp(Function &f, string test_input) {
  
  if (test_input != "None"){
    parse_input(test_input);
  }

  unsigned instr_limit = 500;
    ...
    while (cur_block) {
      ...
      for (auto &i : cur_block->instrs()) {
        ...
        if (dynamic_cast<const BinOp *>(&i)) {
          ...
          cout << "ERROR : alive-interp unsupported instruction. Aborting" << '\n';
          exit(EXIT_FAILURE);
        }
      }
    }
...
void interp_outline(Function &f, ojson& result, string test_input) {
