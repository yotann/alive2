...
struct ConcreteByte {
    bool is_pointer;
    // for now, for non pointer bytes, treat the entire byte as an i8 for poison purposes
    std::variant<ConcreteValPointer, ConcreteValInt> value;
    int pointer_byte_offset;

    ConcreteByte() 
    : is_pointer(false), value(ConcreteValInt(true, llvm::APInt(8, 0))), pointer_byte_offset(0)   
    {}
...
struct ConcreteBlock {
    ...
    bool operator==(ConcreteBlock& rhs) {
      ...
      if ((size != rhs.size) || (address != rhs.address) 
          || (align != rhs.align))
        return false;
